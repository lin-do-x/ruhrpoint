...
console.log(cards)
fetch('php/fetch/fetch_prudacts/fetch_all_prudacts/fetch.php')
        ...
        <div class="bg-gray-200 rounded-xl cursor-pointer hover:scale-[1.03] transition-all relative overflow-hidden">
            ...
            <div class="p-6 bg-white">
              ...
              <div class="w-2/3 h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8">
                <img src="images/prudacts/${row.category}/${row.img_path}" alt="Product 1" class="h-full w-full object-contain" />
              </div>
            </div>
    
            <div class="text-center bg-gray-100 p-6">
              <h3 class="text-lg font-bold text-gray-800">${row.titel} ${row.sub_titel}</h3>
              <h4 class="text-lg text-gray-800 font-bold mt-6">${row.price}
               
              </h4>
    
              <button type="button" onclick="addToCart(this)" data="${row.p_id}"
    ...
    function addToCart(element) {
      ...
      // Assuming 'element' is defined and available in your context
      var attr = parseInt(element.getAttribute('data'));
      
      const xhr = new XMLHttpRequest();
      xhr.open('POST', 'php/cart/add/send.php', true);
      ...
      xhr.send(`attr=${encodeURIComponent(attr)}`);
    }
